...
<div class="x-body">
    <xblock>
        <button class="layui-btn" onclick="x_admin_show('添加数据','<?php echo U('Test/constructAdd');?>?id=<?php echo ($id); ?>',650,500)"><i class="layui-icon"></i>添加数据</button>
    ...
    <script>
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,url: "<?php echo U('Test/constructHome');?>?id=<?php echo ($id); ?>"
                ,cols: [[
            ...
            table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'edit'){ //编辑
                    x_admin_show('施工编辑',"<?php echo U('Test/editConstruct');?>?id="+data.construct_id,650,550);
                }else if(layEvent === 'del'){ //删除
                    layer.confirm('确认删除么', function(index){
                        $.post("<?php echo U('Test/deleteConstruct');?>",{'id':data.construct_id},function(res){
                            if (res.code == 200) {
