...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a>首页</a>
        <a href="<?php echo U('index/vrList');?>">用户列表</a>
      </span>
        ...
        table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'use'){ //启用
                ...
                layer.confirm(tip, function(index){
                    $.post("<?php echo U('auth/changeshow');?>",{'id':data.admin_id,'show':data.admin_show},function(res){
                        if (res.code == 200) {
                ...
                layer.confirm('确认删除么', function(index){
                    $.post("<?php echo U('auth/adminDelete');?>",{'id':data.admin_id},function(res){
                        if (res.code == 200) {
                ...
                x_admin_show('编辑管理员',"<?php echo U('auth/adminEdit');?>?id="+data.admin_id,650);
            }
        });
