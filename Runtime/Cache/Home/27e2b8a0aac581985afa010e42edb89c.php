...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a>首页</a>
        <a href="<?php echo U('index/vrList');?>">日志列表</a>
      </span>
...
<div class="x-body">
    <div class="layui-row">
        <div class="demoTable layui-col-md12 x-so">
            <input type="text" name="select"  placeholder="请输入操作管理员或操作行为" autocomplete="off" class="layui-input" id="demoReload">
...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#LAY_table_user'
            ,url: "<?php echo U('log/logsList');?>"
            ,cols: [[
                {field:'id', title: 'ID',sort: true,align: 'center'}
                ,{field:'url', title: '操作路径',  align: 'center'}
                ,{field:'operator',  title: '操作管理员',  align: 'center'}
                ,{field:'description',  title: '操作行为', align: 'center'}
                ,{field:'operate_time',  title: '时间', sort: true, align: 'center'}
        ...
        table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'del'){ //删除
                layer.confirm('确认删除么,删除后不可恢复！', function(index){
                    $.post("<?php echo U('Log/logsDelete');?>",{'id':data.id},function(res){
                        if (res.code == 200) {
