...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a>首页</a>
        <a href="<?php echo U('index/vrList');?>">权限列表</a>
      </span>
...
<div class="x-body">
    <div class="layui-row">
        <div class="demoTable layui-col-md12 x-so">
            <input type="text" name="select"  placeholder="请输入规则名称" autocomplete="off" class="layui-input" id="demoReload">
            <button class="layui-btn"  lay-submit="" lay-filter="sreach" data-type="reload"><i class="layui-icon">&#xe615;</i></button>
    </div>
</div>
<xblock>
    <button class="layui-btn" onclick="x_admin_show('添加规则','ruleAdd.html',570,300)"><i class="layui-icon"></i>添加规则</button>
...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#LAY_table_user'
            ,url: "<?php echo U('auth/ruleList');?>"
            ,cols: [[
                {field:'id', title: 'ID',sort: true,align: 'center'}
                ,{field:'name', title: '权限规则',  align: 'center'}
        ...
        table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'del'){ //删除
                layer.confirm('确认删除么', function(index){
                    $.post("<?php echo U('auth/ruleDelete');?>",{'id':data.id},function(res) {
                        if (res.code == 200) {
                ...
                x_admin_show('编辑管理员',"<?php echo U('auth/ruleEdit');?>?id="+data.id,570,300);
            }
        });
